...
import org.adawaycn.provider.AdAwayContract.BlacklistColumns;
import org.adawaycn.provider.AdAwayContract.HostsSourcesColumns;
import org.adawaycn.provider.AdAwayContract.RedirectionListColumns;
import org.adawaycn.provider.AdAwayContract.WhitelistColumns;
import org.adawaycn.util.Constants;
import org.adawaycn.util.Log;

public class AdAwayDatabase extends SQLiteOpenHelper {
    private static final String DATABASE_NAME = "adawaycn.db";
...
//        insertHostsSource(insertStmt, "https://adaway.org/hosts.txt");
        insertHostsSource(insertStmt, "https://coding.net/u/Elaine_ni/p/hosts/git/raw/master/hosts.txt");
//        insertHostsSource(insertStmt, "http://www.91sh.cn/hosts.txt");
...
//                    + " (url, last_modified_local, last_modified_online, enabled) VALUES (\"http://adaway.org/hosts.txt\", 0, 0, 1)");
//        }
        if (oldVersion <= 11) { // 9 and 10, forgot to change on new installs
            // change to https
            // remove both
            db.execSQL("DELETE FROM " + Tables.HOSTS_SOURCES
                    + " WHERE url=\"https://coding.net/u/Elaine_ni/p/hosts/git/raw/master/hosts.txt\"");
            // add valid again
            db.execSQL("INSERT INTO "
                    + Tables.HOSTS_SOURCES
                    + " (url, last_modified_local, last_modified_online, enabled) VALUES (\"https://coding.net/u/Elaine_ni/p/hosts/git/raw/master/hosts.txt\", 0, 0, 1)");
        }
//        if (oldVersion <= 11) {
//            // change http://hosts-file.net/ad_servers.asp to http://hosts-file.net/ad_servers.txt
//            db.execSQL("DELETE FROM " + Tables.HOSTS_SOURCES
//                    + " WHERE url=\"http://www.91sh.cn/hosts.txt\"");
//            // add modified entry
//            db.execSQL("INSERT INTO "
//                    + Tables.HOSTS_SOURCES
//                    + " (url, last_modified_local, last_modified_online, enabled) VALUES (\"http://www.91sh.cn/hosts.txt\", 0, 0, 1)");
//        }
            else {
